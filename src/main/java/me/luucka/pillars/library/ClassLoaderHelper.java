...
abstract class ClassLoaderHelper {
	...
	protected void setMethodAccessible(final LibraryManager libraryManager, final Method method, final String methodSignature, final Consumer<MethodHandle> methodHandleConsumer, final Consumer<Instrumentation> instrumentationConsumer) {
		...
		if (unsafeException != null)
			PillarLog.error("Cannot set accessible " + methodSignature + " using unsafe");
		if (javaAgentException != null)
			PillarLog.error("Cannot set accessible " + methodSignature + " using java agent");
		...
		if (moduleName != null)
			PillarLog.warn("Cannot set accessible " + methodSignature + ", if you are using Java 9+ try to add the following option to your java command: --add-opens " + moduleName + "/" + packageName + "=ALL-UNNAMED");
		else
			// In case the try-and-catch above failed, should never happen
			PillarLog.warn("Cannot set accessible " + methodSignature);
